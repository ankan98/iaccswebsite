...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $title = trim($_POST['title'] ?? 'Notices & Announcements');
    $heading = trim($_POST['heading'] ?? 'Notices & Announcements');
    $subheading = trim($_POST['subheading'] ?? 'Stay updated with latest announcements');
    $content = trim($_POST['content'] ?? '');
    $custom_css = trim($_POST['custom_css'] ?? '');
    $type = trim($_POST['type'] ?? 'static');

    $stmt = $conn->prepare("UPDATE cms_pages SET title = ?, heading = ?, subheading = ?, content = ?, custom_css = ?, type = ? WHERE slug = 'notices-announcements'");
    $stmt->bind_param("ssssss", $title, $heading, $subheading, $content, $custom_css, $type);
    
    if ($stmt->execute()) {
        $_SESSION['message'] = 'Notices & Announcements Page updated successfully!';
        $_SESSION['message_type'] = 'success';
    } else {
        $_SESSION['message'] = 'Failed to update Notices & Announcements Page: ' . $conn->error;
        $_SESSION['message_type'] = 'error';
    }
    $stmt->close();

    header("Location: notices-announcements-form.php");
    exit();
}
...
$page_title = 'Notices & Announcements Custom Editor';
include '../include/header.php';

// Fetch page data
$stmt = $conn->prepare("SELECT * FROM cms_pages WHERE slug = 'notices-announcements' LIMIT 1");
$stmt->execute();
$page_data = $stmt->get_result()->fetch_assoc();
$stmt->close();

// Resolve existing details
$title = $page_data['title'] ?? 'Notices & Announcements';
$heading = $page_data['heading'] ?? 'Notices & Announcements';
$subheading = $page_data['subheading'] ?? 'Stay updated with latest announcements';
...
<div class="w-full space-y-6 pb-28">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold text-slate-900 dark:text-white">Notices & Announcements Visual Editor</h1>
            <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Easily configure page title, banner headings, intro content, and custom CSS for Notices & Announcements.</p>
        </div>
    </div>
    ...
    <form id="notices_visual_form" method="POST" action="">
        ...
        <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-sm overflow-hidden">
            <div class="border-b border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50 px-6 py-4">
                <h2 class="text-base font-bold text-slate-900 dark:text-white">Edit Notices & Announcements Page Content</h2>
            </div>
            ...
            <div class="p-6 space-y-8">
                ...
                <div class="space-y-4 pt-4">
                    ...
                    <div class="space-y-2">
                        <label class="block text-sm font-semibold text-slate-700 dark:text-slate-300">Description (Rich HTML editor - shows above notices list)</label>
                        <textarea id="notices_content" name="content" rows="8" class="w-full p-4 rounded-lg border border-slate-300 dark:border-slate-700 bg-transparent text-sm text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-primary transition-all"><?= htmlspecialchars($content) ?></textarea>
                    </div>
                </div>
    ...
    <div class="fixed bottom-0 left-64 right-0 z-40 bg-white/95 dark:bg-slate-900/95 backdrop-blur-sm px-6 py-4 border-t border-slate-200 dark:border-slate-800 flex justify-center shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.03),0_-2px_4px_-1px_rgba(0,0,0,0.02)]">
        <div class="w-full flex justify-end gap-3">
            ...
            <button type="submit" form="notices_visual_form" class="px-8 py-3 bg-primary text-white text-sm font-bold rounded-lg shadow-md shadow-primary/20 hover:bg-primary/95 hover:scale-[1.01] active:scale-[0.99] transition-all">
                Save
            </button>
        </div>
    </div>

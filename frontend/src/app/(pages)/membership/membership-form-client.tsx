...
export default function MembershipFormClient({ initialPageData }: { initialPageData: any }) {
  const [pageData, setPageData] = useState<any>(initialPageData);

  useEffect(() => {
    const apiBase = getPhpBaseUrl();
    fetch(`${apiBase}/get_page.php?slug=membership`)
      ...
      .catch((e) => console.error("Client fetch membership page data error:", e));
  }, []);
  ...
  return (
    <>
      ...
      <section className="bg-gray-50 px-4 sm:px-6 lg:px-[110px] py-6 sm:py-8 mt-16 md:mt-[79px]">
        ...
        >
          {showThankYou ? "Thank You" : (pageData?.title || "Membership Application")}
        </h1>
        {!showThankYou && pageData?.heading && (
          <h2 className="text-xl font-bold text-gray-800 mb-2">
            {pageData.heading}
          </h2>
        )}
        {!showThankYou && pageData?.subheading && (
          <p className="text-gray-700 text-sm md:text-base mb-4 max-w-3xl">
            {pageData.subheading}
          </p>
        )}
        {!showThankYou && pageData?.btn_text && pageData?.btn_link && (
          <div className="mb-4">

...
export default function ContactUsFormClient({ initialPageData }: { initialPageData: any }) {
  ...
  // Pre-fetched pageData passed from Server Component
  ...
  useEffect(() => {
    ...
    fetch(`${apiBase}/get_page.php?slug=contact-us`)
      ...
      .catch((e) => console.error("Client fetch contact-us page data error:", e));
  }, []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(buildPhpUrl("contact_form_submit.php"), {
        method: "POST",
        body: formDataToSend,
      });
  ...
  if (pageData?.contact_json) {
    try {
      contactConfig = typeof pageData.contact_json === "string"
        ? JSON.parse(pageData.contact_json)
        : pageData.contact_json;
    } catch (e) {
      console.error("Failed to parse contact_json:", e);
    }
  }
  const emailAddress = pageData?.official_email || contactConfig?.email || "admin@iaccs.org.in";
  const rawSubheading = pageData?.subheading || "Mathkal, Nazrul Sarani, Dumdum Cantonment, Kolkata, 700065";
  const cleanAddress = rawSubheading.replace(/^Address:\s*/i, '');
    ...
    <div className="bg-white">
      ...
      <section className="w-full bg-white flex flex-col gap-16 py-16 xl:px-[110px] md:px-20 px-5 mt-[40px]">
        <div className="w-full gap-[82px]">
          <h1 className="w-full font-bold text-gray-900 xl:text-[48px] md:text-4xl sm:text-4xl text-3xl !leading-tight mb-[20px]" style={{ fontFamily: "Georgia, serif" }}>
            {pageData?.title || "Contact Us"}
          </h1>
          {pageData?.btn_text && pageData?.btn_link && (
            <div className="mb-4">
              <a href={pageData.btn_link} className="inline-block px-5 py-2 bg-[#38b6ff] text-black font-bold rounded-full border border-black hover:opacity-90 transition text-sm">
                {pageData.btn_text}
              </a>
            </div>
          )}
          {pageData?.content && (
            <div className="mb-6 prose max-w-4xl" dangerouslySetInnerHTML={{ __html: pageData.content }} />
          )}

          <div className="flex flex-wrap -mx-4 gap-y-10">
            <div className="lg:w-1/2 w-full px-4">
                <div className="space-y-4">
                  <h3 className="text-2xl font-semibold mb-4 text-gray-800" style={{color:'#1a4075'}}>
                    {pageData?.heading || "Head Office Address"}
                  </h3>

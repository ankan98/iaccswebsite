...
import { getPhpBaseUrl, resolveAssetUrl } from "@/app/lib/utils";
...
interface FooterClientProps {
  initialSettings: any;
  initialFooterMenuItems: any[];
}

export default function FooterClient({ initialSettings, initialFooterMenuItems }: FooterClientProps) {
  const [settings, setSettings] = useState<any>(initialSettings);
  const [footerMenuItems, setFooterMenuItems] = useState<any[]>(initialFooterMenuItems || []);
  ...
  useEffect(() => {
    ...
    fetch(`${apiBase}/get_settings.php`)
      ...
      .catch((e) => console.error("Client fetch footer settings error:", e));

    // Fetch footer menu dynamically
    fetch(`${apiBase}/get_menus.php?menu_id=2`)
      .then((res) => (res.ok ? res.json() : null))
      .then((data) => {
        if (data && data.items) setFooterMenuItems(data.items);
      })
      .catch((e) => console.error("Client fetch footer menu error:", e));
  }, []);
            ...
            {sortedSocials.length > 0 ? (
              <div className="flex items-center sm:justify-center gap-5 md:gap-6 mb-4">
                {sortedSocials.map(([key, social]) => {
                  const iconSrc = resolveAssetUrl(social.image, STATIC_ICONS[key] || "");
